...
const DashboardModel = {
  ...
  getBranchWiseAnalysis: async (region_id, start_date, end_date, type) => {
    try {
      if (
        type === "Leads" ||
        type === "Follow Up" ||
  ...
  postSalePerformance: async (user_ids, start_date, end_date) => {
    try {
      const queryParams = [];
      let sql = `SELECT SUM(CASE WHEN c.status = 'Awaiting Trainer' THEN 1 ELSE 0 END) AS awaiting_trainer, SUM(CASE WHEN c.status = 'Awaiting Trainer Verify' THEN 1 ELSE 0 END) AS awaiting_trainer_verify, SUM(CASE WHEN c.status = 'Trainer Rejected' THEN 1 ELSE 0 END) AS rejected_trainer, SUM(CASE WHEN c.status = 'Awaiting Class' THEN 1 ELSE 0 END) AS verified_trainer, SUM(CASE WHEN c.status = 'Awaiting Verify' THEN 1 ELSE 0 END) AS awaiting_verify, SUM(CASE WHEN c.status = 'Awaiting Class' THEN 1 ELSE 0 END) AS awaiting_class, SUM(CASE WHEN c.status = 'Class Scheduled' THEN 1 ELSE 0 END) AS class_scheduled, SUM(CASE WHEN c.status = 'Class Going' THEN 1 ELSE 0 END) AS class_going, SUM(CASE WHEN c.google_review IS NOT NULL THEN 1 ELSE 0 END) AS google_review_count, SUM(CASE WHEN c.linkedin_review IS NOT NULL THEN 1 ELSE 0 END) AS linkedin_review_count, SUM(CASE WHEN c.status = 'Completed' THEN 1 ELSE 0 END) AS class_completed, SUM(CASE WHEN c.status IN ('Videos Given', 'Escalated', 'Hold', 'Demo Completed', 'Discontinued', 'Partially Closed', 'Refund') THEN 1 ELSE 0 END) AS others FROM customers AS c INNER JOIN lead_master AS l ON c.lead_id = l.id WHERE 1 = 1`;
  ...
  getRegionWiseAnalysis: async (region_id, start_date, end_date, type) => {
    try {
      if (type === "Sale" || type === "Collection" || type === "Pending") {
        let saleVolumeQuery = `SELECT r.id AS region_id, r.name AS region_name, IFNULL(SUM(pm.total_amount), 0) AS sale_volume FROM region AS r LEFT JOIN customers AS c ON r.id = c.region_id`;
