...
const ReportModel = {
  ...
  reportBranchWiseLeads: async (
    ...
    `;
      ...
      branch_months AS (
        ...
        UNION
        SELECT branch_id, month FROM sale
        UNION
        SELECT branch_id, month FROM collections
        UNION
        SELECT branch_id, month FROM total
      )
      SELECT
        bm.branch_id,
        ...
        IFNULL(s.sale_volume, 0) AS sale_volume,
        IFNULL(c.collection, 0) AS collection,
        IFNULL(t.total_collection, 0) AS total_collection
      ...
      LEFT JOIN sale s ON s.branch_id = bm.branch_id AND s.month = bm.month
      LEFT JOIN collections c ON c.branch_id = bm.branch_id AND c.month = bm.month
  ...
  getTopPerformingReport: async (
    ...
    `;
      ...
      total_result.forEach((r) => {
        ...
        total_lead_count.pending += Number(r.pending);
        total_lead_count.total_collection += Number(r.total_collection);
      });
  ...
  getRegionWiseFinance: async (start_date, end_date, type) => {
    try {
      ...
      if (type === "Region") {
        ...
        chennai_total =
          anna_nagar_total + omr_total + porur_total + velachery_total;
        bangalore_total =
          btm_layout_total + e_city_total + rajaji_nagar + marathahalli_total;
  ...
  getTransactionWiseReport: async (start_date, end_date) => {
    try {
      const getQuery = `SELECT
                        ...
                        FROM (
                            ...
                            WHERE pt.payment_status <> 'Rejected'
                              AND pt.invoice_date BETWEEN ? AND ?
                        ) x
